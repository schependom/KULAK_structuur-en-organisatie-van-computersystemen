...
# FUNCTIE int iskleiner(struct rationaalgetal a, struct rationaalgetal b)
# parameters:    a in    8(fp) -> 16(fp)
#			     b in   20(fp) -> 28(fp)
# lokale variabele: (leaf)
# resultaat: a0
#
iskleiner:							# int iskleiner(struct rationaalgetal a, struct rationaalgetal b) {
			...
			lw		t0, 16(fp)
			lw		t1, 28(fp)
			bge		t0, t1, elseIK1	#	if(a.gehdeel < b.gehdeel)
			li		a0, 1			#		return 1;
			j		retIK
elseIK1:	ble		t0, t1, elseIK2	#	if(a.gehdeel > b.gehdeel)
			li		a0, 0			#		return 0;
			j		retIK
elseIK2:	lw		t0, 8(fp)		
			lw		t1, 24(fp)
			...
			lw		t2, 12(fp)
			mul		t1, t1, t2
			bge		t0, t1, elseIK3	#	if(a.teller*b.noemer < b.teller*a.noemer)
			li		a0, 1			#		return 1;
			j		retIK			#	else
elseIK3:	li		a0, 0			#		return 0;

retIK:		# herstel frame pointer
			lw		fp, 0(sp)		
			addi	sp, sp, 4		 
...
whM:		# call iskleiner
			...
			addi	sp, sp, -4		#
			sw		ra, 0(sp)		#
			call  	iskleiner		#		// a0 = iskleiner(rij[j],rij[in])		#### ISKLEINER IPV ISGELIJK
			lw		ra, 0(sp)		#
			addi	sp, sp, 4		#
			# verwijder param van de stack
			addi	sp, sp, 24
			beqz	a0, endWM		#		while(isGelijk(rij[j],rij[in]) != 0){  #### BEQZ ipv BNEZ !
			addi	s2, s2, 1		#			j++;
			j		whM				#		}
endWM:		beq		s2, s3, endIM	#		if(j != in){
			...
			beqz	a0, elseM		#		if(isGelijk(rij[j],rij[in]) != 0)
			addi	s3, s3, -1		#			in--;
			j		endIM2			#		else{
elseM:		la		t0, rij	
			li		t1, 12
			mul		t1, t1, s3
			add		t1, t0, t1		#			// t1 = &rij[in]
			lw		t0, 0(t1)
			sw		t0, -12(fp)
			lw		t0, 4(t1)
			sw		t0, -8(fp)
			...
			sw		t0, -4(fp)		#			q = rij[in];
			mv		s4, s3			#			t = in;
forM2:		ble		s4, s2, endFM2	#			for(   ; t>j;   ){
			la		t0, rij	
			li		t1, 12
			mul		t1, t1, s4
			add		t1, t0, t1		#				// t1 = &rij[t]
			lw		t0, -12(t1)
			sw		t0, 0(t1)
			lw		t0, -8(t1)
			sw		t0, 4(t1)
			lw		t0, -4(t1)
			sw		t0, 8(t1)		#				rij[t] = rij[t-1];
			addi	s4, s4, -1		#				t--;
			j		forM2			#			}
endFM2:		la		t0, rij	
			li		t1, 12
			mul		t1, t1, s2
			add		t1, t0, t1		#			// t1 = &rij[j]
			lw		t0, -12(fp)
			sw		t0, 0(t1)
			lw		t0, -8(fp)
			sw		t0, 4(t1)
